...
private slots:
    ...
    void addNewTab() {
        ...
        QPushButton* tabBtn = new QPushButton(QString("LiTE (0.0.2x197)"));
    ...
    void handleDownload(const QNetworkRequest& request) {
        ...
        if (!savePath.isEmpty()) {
            ...
            if (file->open(QIODevice::WriteOnly)) {
                QNetworkAccessManager* manager = qobject_cast<QNetworkAccessManager*>(sender()->parent());
                if (!manager) {
